...
public class BreadthFirstSearch  {
	...
	public void start(){	
		while(someVertexUnvisited()) {	
			...
			//performs necessary processing (for subclasses) between numCompletions 
			//of successive numComponents
	...
	protected void singleComponentLoop(){
		while(current != null) {
			List<Vertex> listOfAdjacent = adjacencyList.get(current);
			for(Vertex w : listOfAdjacent) {
				if(!visitedVertices.containsKey(w)) {
					setHasBeenVisited(w);
					processEdge(new Edge(w, current));
					processVertex(w);
					queue.add(w);
				}
			}
...
//		numComp = new NumComponents(graph).additionalProcessing();
//		return numComp;
//        NumComponents num = new NumComponents(graph);
//        num.additionalProcessing();
//        numComp = num.getNumComponents();
        new NumComponents(graph).additionalProcessing();
	}

	protected void processEdge(Edge e) {
		//override for needed functionality
		//new EdgeComponent(graph).processEdge(e);
	}
	
	protected void processVertex(Vertex w){
		//should be overridden; by default, do nothing
		//new VertexComponent(graph).processVertex(w,numComp);
	}
